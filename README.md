...
코어 진입점인 `Prm` 프로토콜은 `PrmFactory.getInstance()` 싱글턴으로 획득하며, 진출입 결과는 `PrmCallback` 으로 통지된다.
...
### 2. Prm 인스턴스 획득 및 콜백 등록
`PrmFactory.getInstance()` 로 싱글턴을 얻고, `PrmCallback` 구현체를 등록한다.
...
final class MyPrmService: PrmCallback {
    ...
    init() {
        prm.setCallback(callback: self)   // 콜백은 start 이전에 등록
    }
...
service.prm.start(areaInfoList: [area], wallInfoList: [])

// 측위 좌표 주입 → 영역 안이면 onReceivedInout("IN", ...)
service.prm.pushEvent(spaceId: 1, tagId: "TAG-001", x: 5, y: 5, z: 0)
...
### 팩토리: `PrmFactory`
* **`static func getInstance() -> Prm`**
  * PRM 엔진의 싱글턴 인스턴스(`Prm`)를 반환한다.

### 프로토콜: `Prm`
PRM 엔진 컨트롤러. 진출입 판정의 모든 제어 진입점이다.

* **`func setCallback(callback: PrmCallback)`** — 진출입/상태 이벤트를 받을 콜백 등록. (start 이전 호출)
...
### 버전: `SDKConfig`
* **`static var version: String`** — 현재 SDK 버전. framework Info.plist 의 `CFBundleShortVersionString`(빌드 시 Xcode `MARKETING_VERSION`)을 읽어 반환한다. (예: `"1.1.0"`)
